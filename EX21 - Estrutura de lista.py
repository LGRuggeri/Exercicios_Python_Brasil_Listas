...
# custe R$ 2,25 o litro. Abaixo segue uma tela de exemplo. 
...
for i in range(5):
    veiculo= input(f'Digite o {i+1} veículo: ')
    lista_veiculos.append(veiculo)
    km_litro= float(input('Digite o consumo por litro do veículo: '))
    consumo_km.append(km_litro)
    clear= lambda: os.system('cls')
    clear()

clear()
preco_combustivel= float(input('Digite o valor do combustível: '))
distancia_percorrida= float(input('Digite a distancia a ser percorrida: '))
clear()

for i in range(len(lista_veiculos)):
    print(f'Veículo {i+1}','\nNome:',lista_veiculos[i],'\nKm por litro: ',consumo_km[i])
print('\nRelatório Final')
for i in range(len(lista_veiculos)):
    media_veiculo= (round(distancia_percorrida / consumo_km[i]))
    gasto_combustivel= (round(preco_combustivel * media_veiculo))
    print(i+1,' - ',lista_veiculos[i],' - ',consumo_km[i],' - ',media_veiculo,'litros',' - ','R$',gasto_combustivel)    